defmodule SkyLoyaltyTest do
  ...
  test "earns and redeems points deterministically" do
    ...
    assert {:ok, ledger, %{balance: 200}} = SkyLoyalty.apply(ledger, "tx-1", "member-1", :earn, 200)
    assert {:ok, ledger, %{balance: 125}} = SkyLoyalty.apply(ledger, "tx-2", "member-1", :redeem, 75)
  ...
  test "rejects duplicate transaction ids without mutating the ledger" do
    ...
    assert {:error, "transaction_id already applied"} = SkyLoyalty.apply(ledger, "tx-1", "member-1", :earn, 100)
  ...
  test "rejects redemption that exceeds the balance" do
    ...
    assert {:error, "insufficient points"} = SkyLoyalty.apply(ledger, "tx-2", "member-1", :redeem, 51)
    assert {:ok, 50} = SkyLoyalty.balance(ledger, "member-1")
  end

  test "supports bounded positive and negative adjustments" do
    ledger = SkyLoyalty.new()
    assert {:ok, ledger, %{balance: 100}} = SkyLoyalty.apply(ledger, "tx-1", "member-1", :adjust, 100)
    assert {:ok, ledger, %{balance: 60}} = SkyLoyalty.apply(ledger, "tx-2", "member-1", :adjust, -40)
    assert {:error, "balance cannot become negative"} = SkyLoyalty.apply(ledger, "tx-3", "member-1", :adjust, -61)
