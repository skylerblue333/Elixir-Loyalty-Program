defmodule SkyLoyalty.CLI do
  ...
  def main(["demo"]) do
    ...
    with {:ok, ledger, earned} <- SkyLoyalty.apply(ledger, "tx-001", "member-001", :earn, 250),
         {:ok, ledger, redeemed} <-
           SkyLoyalty.apply(ledger, "tx-002", "member-001", :redeem, 75) do
  ...
  def main(["validate", transaction_id, member_id, operation, points_text]) do
    ...
    with {:ok, points} <- parse_points(points_text),
         {:ok, _ledger, result} <-
           SkyLoyalty.apply(SkyLoyalty.new(), transaction_id, member_id, operation, points) do
      IO.puts(
        "accepted transaction_id=#{result.transaction_id} member_id=#{result.member_id} operation=#{result.operation} balance=#{result.balance}"
      )
    else
      {:error, reason} -> fail(reason)
    end
  ...
  def main(_args) do
    ...
    IO.puts(
      "usage: sky_loyalty demo | validate <transaction_id> <member_id> <earn|redeem|adjust> <points>"
    )
  end
